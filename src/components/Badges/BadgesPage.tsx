      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 lg:px-8">
        ...
        <div className="text-center mb-6 sm:mb-8">
          <div className="flex items-center justify-center mb-4">
            ...
            <h1 className="text-2xl sm:text-3xl font-bold text-gray-900">Badges & Achievements</h1>
          </div>
        ...
        <div className="grid grid-cols-2 gap-3 sm:gap-6 mb-6 sm:mb-8">
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-3 sm:p-6 text-center">
            <Award className="w-6 h-6 sm:w-8 sm:h-8 text-yellow-500 mx-auto mb-2" />
            <h3 className="text-lg sm:text-2xl font-bold text-gray-900">{userBadges.length}</h3>
            <p className="text-gray-600 text-xs sm:text-base">Badges Earned</p>
          </div>
          <div className="bg-white rounded-xl shadow-sm border border-gray-100 p-3 sm:p-6 text-center">
        ...
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 2xl:grid-cols-8 gap-3 sm:gap-4">
          {userBadges.length > 0 ? (
            ...
            <div className="col-span-full text-center py-16">
              ...
              <p className="text-gray-500">Start completing tasks to earn your first badge!</p>
            </div>
          )}
        </div>
      </div>
...
function BadgeCard({ badge, earned }: { badge: Badge; earned: boolean }) {
  return (
    <div className={`group relative bg-white rounded-xl shadow-sm border border-gray-100 p-3 sm:p-4 text-center transition-all hover:shadow-lg hover:scale-105 cursor-pointer ${
      !earned ? 'opacity-60' : ''
    }`}>
      {/* Tooltip - Hidden on mobile, visible on hover for larger screens */}
      <div className="absolute invisible group-hover:visible opacity-0 group-hover:opacity-100 transition-all duration-300 bg-gray-900 text-white text-xs rounded-lg p-3 -top-2 left-1/2 transform -translate-x-1/2 -translate-y-full z-20 w-56 shadow-xl border border-gray-700 hidden sm:block">
        <div className="font-semibold mb-1 text-yellow-300">{badge.name}</div>
        <div className="text-gray-200 mb-2 leading-relaxed">{badge.description}</div>
        {earned && badge.earned_at && (
          <div className="text-green-300 text-xs border-t border-gray-700 pt-2">
            ✅ Earned: {new Date(badge.earned_at).toLocaleDateString()}
          </div>
        )}
        {!earned && badge.criteria && (
          <div className="text-blue-300 text-xs border-t border-gray-700 pt-2">
            🎯 How to earn: {' '}
      ...
      {/* Badge Name */}
      <h3 className="font-bold text-xs sm:text-sm text-gray-900 mb-1 leading-tight">{badge.name}</h3>
      
      {/* Description - Only on larger screens */}
      <p className="text-xs text-gray-600 mb-2 line-clamp-2 hidden xl:block leading-tight">{badge.description}</p>
      
      {/* Date for earned badges */}
      {earned && badge.earned_at && (
        <div className="flex items-center justify-center text-xs text-green-600 bg-green-50 rounded-full px-2 py-1 mx-auto w-fit">
      ...
      {!earned && (
        <div className="text-xs text-gray-400 bg-gray-50 rounded-full px-2 py-1 mx-auto w-fit">
